...
function buildAdminFormConfig() {
    return [
        ...
        'password' => ['type'=>'text','regex'=>'password','label'=>'Password','name'=>'password','id'=>'password','errorMsg'=>'You must enter a valid password.','error'=>'','required'=>true,'value'=>''],
        'status'   => ['type'=>'select','label'=>'Status','name'=>'status','id'=>'status','errorMsg'=>'You must select a status.','error'=>'','required'=>true,'selected'=>'','options'=>['0'=>'Please Select a Status','staff'=>'staff','admin'=>'admin']]
    ];
}
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($formConfig['masterStatus']['error'] === false) {
            ...
            if ($existing === 'error') {
                ...
                $bindings = [
                    [':name',$fullName,'str'],[':email',$emailVal,'str'],
                    [':password',$hashedPw,'str'],[':status',$statusVal,'str']
                ];
